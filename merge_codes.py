...
df_just = pd.read_csv("justifications_long_parsed.csv")

# Upload dates coded file
df_dates = pd.read_csv("dates_long_parsed.csv")
df_dates = df_dates[['date', 'img_file']]

# Upload date range (based on file archive record) file
df_date_range = pd.read_csv("date_range.csv", dtype=str)
...
# df_merge_final = pd.DataFrame.merge(df_merge, df_date_range, 'left', on="file_id")

# output csv
df_merge_final = df_merge_final[['img_file', 'file_id_orig', 'file_id', 'image_id', 'start_yr', 'start_mo', 'end_year', 'end_mo', 'justification', 'raw_text', 'just_plain_text_all_ref', 'just_text_all_ref', 'ref_count', 'coded_refs']]
