...
class NNModel(torch.nn.Module):
    def __init__(self, pretrained_embeddings, num_output_labels, label_weights=None, process_context_separately=True,
                 num_hidden_layers=3, dim_of_hidden_layers: List[int] = (200, 150, 50)):
        super().__init__()
        assert num_hidden_layers == len(dim_of_hidden_layers)
        self.embeddings = torch.nn.Embedding(num_embeddings=pretrained_embeddings.shape[0],
                                             embedding_dim=pretrained_embeddings.shape[1], padding_idx=0)
        ...
        if process_context_separately:
            dim_of_hidden_layers = [pretrained_embeddings.shape[1] * 2] + dim_of_hidden_layers
        else:
            dim_of_hidden_layers = [pretrained_embeddings.shape[1]] + dim_of_hidden_layers
        ...
        self.loss = torch.nn.CrossEntropyLoss(weight=label_weights)
...
def load_in_pickled_model(dir_to_load, cuda_device, num_labels, label_weights, lowercase_all_text,
                          num_embeddings, embedding_dim, process_context_separately):
    model_to_return = NNModel(np.zeros((num_embeddings, embedding_dim), dtype=float), num_output_labels=num_labels,
...
def train(train_dataset, model, calc_binary_precrec_too, f1_avg_type, seed, n_gpu, max_steps,
          ...
          words_to_inds, lowercase_all_text, regular_exp=None):
    """ Train the model """
...
def load_and_cache_examples(data_df, lowercase_all_text, words_to_inds, includes_context_column=False, padding_token=0,
                            regular_exp=None):
...
def train_and_save_word_embeddings(training_data_df, directory_for_filename,
    ...
    trained_model = Word2Vec(None, iter=iterations_for_training_word_embeddings,
                             min_count=1, size=word_embedding_dimension, workers=4)
    ...
    if run_check:
        ...
        print(part_to_query.most_similar(positive=['faulkner' if lowercase_all_text else 'Faulkner'], negative=[])[:10])
    ...
    for word_ind in inds_to_words.keys():
...
def run_classification(train_df, dev_df, lowercase_all_text, output_dir, num_labels,
                       batch_size: int, learning_rate: float, label_weights: List[float]=None,
                       embedding_dim=100,
                       iterations_for_pretraining_word_embeddings=10, cuda_device=-1, f1_avg='weighted', test_set=None,
    ...
    model = NNModel(pretrained_embeddings, num_labels, label_weights=label_weights,
    ...
    if do_train and True:  # (local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = load_in_pickled_model(dir_to_load, cuda_device, num_labels, label_weights,
                                                 lowercase_all_text, num_embeddings, embedding_dim,
                                      process_context_separately=process_context_separately)
    ...
    if print_results:
        if also_report_binary_precrec:
            print(string_prefix + 'With batch size ' + str(batch_size) + ', learning rate ' + str(learning_rate) +
                  ...
                  ' context features, word2vec baseline result: accuracy is ' + str(acc) + ' and ' + f1_avg +
                  ' f1 is ' + str(f1) +
...
def run_best_model_on(output_dir, test_df, num_labels, label_weights, lowercase_all_text, process_context_separately,
    ...
    if print_results:
        if also_report_binary_precrec:
            print(string_prefix + 'word2vec baseline result: accuracy is ' + str(result['acc']) + ' and ' +
                  f1_avg + ' f1 is ' +
